...
try:
    def game_init():
        ...
        allow_debug=2 #Some debug commands are hidden behind this flag, and ExitError spits out a load of debug info upon exiting
        allow_test=1 #Allows for some test features to be enabled.
    ...
    def age_up():
        ...
        if mother_age == mother_end_age:
            div()
            print("Your mother died.")
            array=["They wish to be buried.","They wish to be cremated.","The wish to be taxidermied.","They wish to have their organs donated."]
            print(array[rng(0,3)])
            print("It is your responsibility to plan the funeral.")
            print("[0] Skip")
            print("[1] Bury [£8500]")
    ...
    def adult_load():
        ...
        adult()
    def debug_log():
       global app_version, sub_version, game_name, version
       global allow_debug
       global happiness, health, intel, looks, ethics, money, forename, surname, age, work_e, edu_lvl, work_e_base, promo_base, degree, hours, salary
       global expenses, pension, is_depressed, debt, savings, year, is_job, stress, mother_name, father_name, mother_age, father_age, mother_end_age, father_end_age, mother_rel, father_rel
       global auto_deposit
...
# This debug log is created automatically using the
# OpenLife Debug Log Creation Tool
# This tool is (c) 2022 WinFan3672 [MIT License]
'''
       print("Creating log...")
       log+=f"{game_name} Debug Log\n"
       log+=f"# Game Info\n"
       log+=f"Version: {version}\n"
       log+=f"AppVersion: {app_version}\n"
       log+=f"SubVersion: {sub_version}\n"
       log+=f"\n# Basic Information\n"
       log+=f"\nLife Name: {forename} {surname}\n"
       log+=f"Age: {age}\n"
       ...
       log+=f"ScreenWidth={screen_width}\n"
       log+=f"ScreenHeight={screen_height}\n"
       log+=f"PyVer={v}\n"
       log+=f"CPU={cpu}\n\n"
       log+=f"# This debug log is created automatically using the\n"
       log+=f"# {game_name} Debug Log Creation Tool\n"
       log+=f"# This tool is (c) 2022 WinFan3672 [MIT License]"
       from time import ctime
       sn=ctime()+".log"
       sn=sn.replace(":","-")
       sn=sn.replace(" ","_")
    ...
    def adult_pause():
        ...
        global edu_lvl, degree, mother_age, father_age, alt_ui, alt_logo, auto_deposit
        div()
        print("Game Paused.")
        div()
        print("[0] Resume")
        ...
        if ch == 0:
           ...
           adult()
            ...
            print("[10] Set Degree")
            print("[11] Set AltUI")
            print("[12] Set AltLogo")
            print("[13] Set AutoDeposit")
            div()
            print("[0] Exit")
            try:
                ch=int(input(">"))
